...
class FigureVisualiser:
    ...
    def _plot_configure(self):
        ...
        spec = plt.GridSpec(nrows=3, ncols=2, width_ratios=[5, 1],
                            height_ratios=[6, 5, 1], wspace=0.3)
        ...
        axes_stick.set(xlabel='ailerons [-]', ylabel='elevator [-]',
                      xlim=(-1, 1), ylim=(-1, 1), xticks=[-1, 1], yticks=[-1, 1])
        ...
        axes_throttle.set(ylabel='throttle [-]', ylim=(0, 1), xlim=(0, 1), yticks=[0, 0.5, 1])
        ...
        axes_rudder.set(xlabel='rudder [-]', xlim=(-1, 1), ylim=(0, 1), xticks=[-1, 0, 1])
        ...
        cmd_line = plt.Line2D([], [], color='b', marker='o', ms=10, linestyle='', fillstyle='none')
        pos_line = plt.Line2D([], [], color='r', marker='+', ms=10, linestyle='')
        figure.legend([cmd_line, pos_line],
    ...
    def _prepare_state_printing(self, ax: plt.Axes):
        ...
        for prop, y in zip(self.print_props, ys):
            label = str(prop.name)
            ax.text(
                constants.TEXT_X_POSN_LABEL, y, label,
                transform=ax.transAxes, **constants.LABEL_TEXT_KWARGS
            )
